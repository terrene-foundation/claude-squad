...
def refresh_account_quota(account_num):
    ...
    try:
        ...
        # Sandboxed: pass refresh token — Claude Code handles token refresh internally
        # --bare: no hooks, no CLAUDE.md, no auto-memory
        # -p ".": single-shot, 1 token, exits immediately
        env = os.environ.copy()
        # Use refresh token (1-year lifetime) — Claude Code refreshes access token internally
