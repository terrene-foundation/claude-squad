...
### 1. csq Is a Terrene Foundation Tool

csq is owned by the Terrene Foundation (Singapore CLG). It is an independent open-source tool for managing Claude Code multi-account rotation. All design decisions must be made on the tool's own merits — what is best for its users and the open-source community.

**Why**: csq is Foundation-owned and irrevocably open. It has no structural relationship with any commercial entity.
...
**How to apply**: If you find yourself writing "unlike X" or "the open-source version of Y" — stop. Describe what csq does on its own terms.

### 3. Foundation-Only and Upstream Dependencies

csq MUST depend only on:
...
MUST NOT describe csq as "the open-source version of [anything]" or "the [X] port of [anything]." csq IS the tool — it is not a derivative, port, or counterpart.

**Correct**: "csq is a Terrene Foundation tool for Claude Code multi-account rotation."
**Incorrect**: "csq is the open-source version of [product name]."

**Why:** "Open-source version of X" framing positions csq as derivative of a commercial product and creates an implicit trademark entanglement that does not legally exist.
...
**Why**: csq's only upstream is Claude Code itself, which is Anthropic's official CLI. That integration is inherent. Every other integration is gratuitous coupling.
...
**Why:** Every proprietary-shaped abstraction constrains future architecture decisions and ties csq's evolution to a product it has no agreement with.
